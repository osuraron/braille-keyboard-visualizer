...
import { createDotToKeyMap, formatKeyLabel } from "../../config";
import type {
  CameraViewName,
  GeometrySettings,
  KeyBindingSpec,
...
const MODE_OPTIONS: readonly { value: ModeName; label: string }[] = Object.freeze([
...
const CAMERA_OPTIONS: readonly { value: CameraViewName; label: string }[] = Object.freeze([
  { value: "ergonomic", label: "Ergonomic" },
  { value: "top", label: "Top" },
  { value: "side", label: "Side" },
]);

const DOT_ROWS: readonly { dots: number[]; description: string }[] = Object.freeze([
  {
    dots: [1, 2, 3],
...
const KEY_BINDINGS: readonly KeyBindingSpec[] = Object.freeze([
...
export interface ControlPanelProps {
  ...
  onGeometryChange: (key: keyof GeometrySettings, value: number) => void;
  onOverlayChange: (
    key: "showSides" | "showNumbers" | "showLetters",
    value: boolean
  ) => void;
  onKeyBinding: (
  ...
  onResetKeymap: () => void;
  onCameraView: (view: CameraViewName) => void;
  onSnapshot: () => void;
}
    ...
    <aside className="control-panel" aria-label="Keyboard controls">
      ...
      <PanelSection title="Geometry">
        <RangeControl
          ...
          onChange={(value) => onGeometryChange("indent", value)}
        />
        ...
        <RangeControl
          ...
          onChange={(value) => onGeometryChange("angleDeg", value)}
        />
        <RangeControl
          label="Key diameter"
          ...
          onChange={(value) => onGeometryChange("keyDia", value)}
        />
      </PanelSection>
      ...
      <PanelSection title="Overlays">
        <CheckboxControl
          ...
          onChange={(value) => onOverlayChange("showSides", value)}
        />
        <CheckboxControl
          label="Dot numbers (1–6)"
          checked={settings.showNumbers}
          onChange={(value) => onOverlayChange("showNumbers", value)}
        />
        <CheckboxControl
          label="Letter mapping (A–J preview)"
          checked={settings.showLetters}
          onChange={(value) => onOverlayChange("showLetters", value)}
        />
      </PanelSection>
        ...
        <div className="keymap-summary">
          {DOT_ROWS.map((row) => (
            <div className="keymap-summary__row" key={row.description}>
              {row.dots.map((dot) => (
                <kbd key={dot}>{formatKeyLabel(dotToKey[dot])}</kbd>
              ))}
          ...
          {KEY_BINDINGS.map((binding) => {
            ...
            return (
              <KeymapField
                ...
                onFocus={() => {
                  setStatus(
                    "Press a printable key for " +
                      (binding.label || "").toLowerCase() +
                      ".",
                    false
                  );
                }}
                onKeyDown={(event) => handleKeyBinding(binding, event)}
              />
            );
          })}
        </div>
        ...
        <div className="segmented-control" aria-label="Camera view">
          {CAMERA_OPTIONS.map((option) => (
            <button
              className={
                "segmented-control__button" +
                (cameraView === option.value ? " is-active" : "")
              }
              type="button"
              aria-pressed={cameraView === option.value}
              key={option.value}
              onClick={() => onCameraView(option.value)}
            >
              {option.label}
            </button>
          ))}
...
function KeymapField({ binding, value, onFocus, onKeyDown }: KeymapFieldProps) {
